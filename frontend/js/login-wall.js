/* ============================================================
   login-wall.js
   Shows login/signup modal after 30 seconds for non-logged-in users
   ============================================================ */

(function() {
  // Don't show on checkout, payment-success, login pages
  ...
  if (localStorage.getItem('learner_token')) return;

  // Don't show more than once per session
  if (sessionStorage.getItem('login_wall_shown')) return;
  ...
  function createModal() {
    ...
    style.textContent = `
      ...
      .lw-close {
        ...
        font-size: 18px; color: #9999b0; line-height: 1;
      ...
      .lw-sub { font-size: 14px; color: #6b6b8a; line-height: 1.6; margin-bottom: 24px; }
      .lw-btn-primary {
        ...
        box-shadow: 0 6px 20px rgba(200,18,64,.3);
      ...
      .lw-skip { font-size: 12px; color: #9999b0; margin-top: 14px; cursor: pointer; }
    ...
    overlay.innerHTML = `
      <div class="lw-card">
        ...
        <p class="lw-sub">Create a free account to access study resources, track your progress, and stay updated on upcoming batches.</p>
        ...
        <div class="lw-skip" id="lwSkip">Continue browsing &rarr;</div>
      </div>
    `;
    ...
    function close() {
      ...
      sessionStorage.setItem('login_wall_shown', '1');
    }
  ...
  setTimeout(function() {
    if (!localStorage.getItem('learner_token') && !sessionStorage.getItem('login_wall_shown')) {
      createModal();
    }
  }, 30000); // 30 seconds
